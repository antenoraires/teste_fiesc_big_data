...
from fiesc_big_data.etl import (
    read_csv_from_minio,
    clear_text,
    render_sql_template,
    execute_rules,
...
class Pipeline:
    ...
    def transform(self, minio_client, config):
        ...
        print(100 * "=")
        df_cursos = read_csv_from_minio(
            minio_client, bucket_name, pasta_destino_sheets + "cursos.csv"
        )
        ...
        df_disciplinas = read_csv_from_minio(
            minio_client, bucket_name, pasta_destino_sheets + "disciplinas.csv"
        )
        ...
        df_disciplinas["avaliacao"] = (
            df_disciplinas["avaliacao"].replace("", "0").astype(int)
        )
        df_disciplinas = df_disciplinas.applymap(clear_text)

        df_turmas = read_csv_from_minio(
            minio_client, bucket_name, pasta_destino_sheets + "turmas.csv"
        )
        df_turmas = df_turmas.iloc[:, 0].str.split(";", expand=True)
        df_turmas.columns = colunas_turmas
        df_turmas["data_inicio"] = pd.to_datetime(
            df_turmas["data_inicio"], format="%Y-%m-%d"
        )
        ...
        df_matricula_aula = read_csv_from_minio(
            minio_client, bucket_name, pasta_nativos + "matricula_aulas.csv"
        )

        df_matriculas_disciplina = read_csv_from_minio(
            minio_client, bucket_name, pasta_nativos + "matricula_disciplinas.csv"
        )

        df_matriculas_notas = read_csv_from_minio(
            minio_client, bucket_name, pasta_nativos + "matricula_notas.csv"
        )

        df_turma_avaliacao = read_csv_from_minio(
            minio_client, bucket_name, pasta_nativos + "turma_avaliacoes.csv"
        )
        ...
        delta_lake_minio(
            minio_client, bucket_name, camada, "disciplinas", df_disciplinas
        )
    ...
    def load(self, engine, spark):
        ...
        df.write.format("jdbc").option("url", db_url).option(
            "dbtable", "matriculas"
        ).option("user", connection_url.username).option(
            "password", connection_url.password
