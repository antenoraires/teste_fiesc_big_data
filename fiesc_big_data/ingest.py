...
def ingest_sheets(path_credentials: str, name_table: str, index: int):
    scope = [
    ...
    print(
        f"Extraindo da tabela {name_table} o index {index} onde;",
        "\n\t0 - Cursos",
        "\n\t1 - Disciplinas",
        "\n\t2 - Turmas",
    )
...
def save_minio(minio_client, bucket_name, pasta_destino, arquivo):
    ...
    # Faz o upload
    minio_client.fput_object(bucket_name, objeto_nome, arquivo)
